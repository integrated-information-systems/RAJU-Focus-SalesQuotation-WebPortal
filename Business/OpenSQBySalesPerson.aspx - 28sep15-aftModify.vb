...
Partial Class Business_OpenSQBySalesPerson
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        'added by ivan - 28sep15
        SqlDataSource1.SelectCommand = "SELECT T1.DocDate,T1.DocNum,T1.CardCode,T1.CardName,T1.DocTotal as 'Amount',T3.DESCRIPT AS 'Territory', T4.CreatedBy FROM [" & SAPSvr & "].[" & SAPDB & "].[dbo].[OQUT] T1 INNER JOIN [" & SAPSvr & "].[" & SAPDB & "].[dbo].[OCRD] T2 ON T2.CardCode=T1.CardCode INNER JOIN [" & SAPSvr & "].[" & SAPDB & "].[dbo].[OTER] T3 ON T3.territryID = T2.Territory INNER JOIN [" & CustomDb & "].[dbo].[SqHeader] T4 ON T4.DocEntry= T1.DocEntry and T4.DocStatus is NULL WHERE T4.CreatedBy=@CreatedBy"
        'this is the correct one - 28sep15
