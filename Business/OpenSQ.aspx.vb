...
Partial Class Business_OpenSQ
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Business_OpenSQ.FocusDBConn.State = 0 Then
            Business_OpenSQ.FocusDBConn.ConnectionString = ConfigurationManager.ConnectionStrings("Focus_CRM_DB_ConnectionString").ConnectionString
            Business_OpenSQ.FocusDBConn.Open()
        End If
        If Business_OpenSQ.SAPDBConn.State = 0 Then
            Business_OpenSQ.SAPDBConn.ConnectionString = ConfigurationManager.ConnectionStrings("SAP_DB_ConnectionString").ConnectionString
            Business_OpenSQ.SAPDBConn.Open()
        End If
        UpdateSQs()
        ...
        If ListofDocs <> String.empty Then
            SqlDataSource1.SelectCommand = "SELECT CASE WHEN ISNULL(docnum,'')='' THEN id ELSE DocNum END AS  DocNum  , [DocStatus], [CardCode], [CardName], [ValidUntil], [DocDate] FROM [SqHeader] WHERE ([DocStatus] IS NULL) AND [id] IN (" + GetListofDocs() + ")  AND [CardName] LIKE @CardName order by docnum desc"
        Else
            SqlDataSource1.SelectCommand = "SELECT CASE WHEN ISNULL(docnum,'')='' THEN id ELSE DocNum END AS  DocNum  , [DocStatus], [CardCode], [CardName], [ValidUntil], [DocDate] FROM [SqHeader] WHERE ([DocStatus] IS NULL) AND createdBy=@createdby  AND [CardName] LIKE @CardName order by docnum desc"
    ...
    Public Function GetListofDocs() As String
        ...
        Dim SAPComm As New SqlCommand("SELECT id, CardCode FROM SqHeader " & _
                                "WHERE  DocStatus is Null  ", SAPConn)
        SAPConn.Open()
    ...
    Protected Function AccessibleDoc(ByVal BPCode As String) As Boolean
        ...
        ReaderCommand.Connection = Business_OpenSQ.SAPDBConn
    ...
    Protected Overridable Overloads Sub Dispose(ByVal disposing As Boolean)
        ...
        If disposing Then
            ...
            Business_OpenSQ.FocusDBConn.Close()
            Business_OpenSQ.SAPDBConn.Close()
    ...
    Protected Sub UpdateSQs()
        ...
        CRMReaderCommand.Connection = Business_OpenSQ.FocusDBConn
        ...
        Dim ReaderCommand As SqlCommand = New SqlCommand()
        ReaderCommand.Connection = Business_OpenSQ.SAPDBConn
        ReaderCommand.CommandText = "SELECT DocNum from OQUT " & _
        ...
        UpdateCRMReaderCommand.Connection = Business_OpenSQ.FocusDBConn
