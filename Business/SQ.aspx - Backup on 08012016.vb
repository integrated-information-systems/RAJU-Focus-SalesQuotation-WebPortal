...
Partial Class Business_SQ
    ...
    Protected Overridable Overloads Sub Dispose(ByVal disposing As Boolean)
        ...
        If disposing Then
            ...
            Session("TempDataset").Dispose()
            Session("TempDataTable").Dispose()
        End If
    ...
    Protected Sub InitializeTempDataTable()
        ...
        SqlAdapter.FillSchema(Session("TempDataset"), SchemaType.Source, "SqLineItems")
        Session("TempDataTable") = Session("TempDataset").Tables("SqLineItems")

        GridView1.DataSource = Session("TempDataTable")
        GridView1.DataBind()
        SqlAdapter.Dispose()
        ...
        If readerHeader.Read() Then
            ...
            If User.IsInRole("SameTerritoryAccess") Then
                If AccessibleDoc(Lookup) = False Then
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "GPMS", "$(document).ready(function () { alert('" & "There is no Docment found" & "'); });", True)
            ...
            For Each row As DataRow In dt.Rows
                Dim DRow As DataRow = Session("TempDataset").Tables("SqLineItems").NewRow()
                DRow("SNo") = row("SNo")
                ...
                Session("TempDataTable").Rows.Add(DRow)
    ...
    '    'Dim a As DataRow = Session("TempDataset").Tables("SqLineItems").NewRow()
    '    'a("ItemCode") = "Raja"
    ...
    Protected Sub btnAction_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        If Page.IsValid Then
            ...
            Select Case btnAction.Text
                Case "Add"
                    DRow = Session("TempDataset").Tables("SqLineItems").NewRow()
                    ...
                    End If
                Case "Update"
                    DRow = Session("TempDataset").Tables("SqLineItems").Rows(Session("SelectedRow"))
                    Rowcount = Session("SelectedRow") + 1
    ...
    Protected Sub LoadChildItems(ByVal ParentItem As String)
        ...
        Dim reader As SqlDataReader = SAPComm.ExecuteReader()
        Session("SelectedRow") = Session("SelectedRow") + 1
        While reader.Read()
            'DRow = Session("TempDataset").Tables("SqLineItems").NewRow()
            ...
            Select Case btnAction.Text
                Case "Add"
                    ...
                    Rowcount = Session("TempDataTable").Rows.Count + 1
                    DRow("SNo") = Rowcount
                Case "Update"
                    DRow = Session("TempDataset").Tables("SqLineItems").Rows(Session("SelectedRow"))
                    Rowcount = Session("SelectedRow") + 1
    ...
    Protected Sub GridView1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView1.SelectedIndexChanged
        ...
        TempDataRow = Session("TempDataTable").Rows(GridView1.SelectedIndex)
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If Session("SelectedRow") <> -1 Then
            'DeleteChildItems(Session("SelectedRow") )
            Dim DRow As DataRow = Session("TempDataset").Tables("SqLineItems").Rows(Session("SelectedRow"))
            DRow.Delete()
            ClearLineItemboxes()
            LoadGridData()
            Session("SelectedRow") = -1
    ...
    Protected Sub DeleteChildItems(ByVal SelectedRow As Integer)
        ...
        Dim DRow As DataRow = Session("TempDataset").Tables("SqLineItems").Rows(SelectedRow)
        While Not IsDBNull(DRow("IsChild"))
            DeleteIndex.Add(SelectedRow)
            SelectedRow = SelectedRow + 1
            If Session("TempDataset").Tables("SqLineItems").Rows().Count > SelectedRow Then
                DRow = Session("TempDataset").Tables("SqLineItems").Rows(SelectedRow)
            Else
                Exit While
            End If
        End While
        ...
        For Index As Integer = 0 To DeleteIndex.Count - 1
            ...
            DRow = Session("TempDataset").Tables("SqLineItems").Rows(val)
            DRow.Delete()
        Next
    End Sub
    Protected Sub LoadGridData()
        ...
        For Each dr As DataRow In Session("TempDataTable").Rows
            Dim LineTotalBeforeDiscount As Double = 0
            Dim LineTotalAfterDiscount As Double = 0
            Dim Qunatity As Integer = 0
            Dim TaxPercent As Double = 0
        ...
        'Sum = Session("TempDataTable").Compute("Sum(LineTotal)", "")
        Sum = DocTotal
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        Dim UpdateCommand As SqlCommand = New SqlCommand()
        UpdateCommand.Connection = Business_SQ.FocusDBConn
        UpdateCommand.CommandText = "UPDATE SqHeader SET SubmitToSAP=1   " & _
                                 "WHERE SqHeader.id=@LookUP"
    ...
    Protected Function SaveSQHeaderNew() As Integer
        Try
            ...
            InsertCommand.CommandText = " Insert into  SqHeader " & _
            "([CardCode]" & _
           ",[CardName]" & _
           ",[ContactPerson]" & _
           ",[CustRefNo]" & _
    ...
    Protected Function SaveSQHeader() As Integer
        ...
        SqlAdapterHeader.FillSchema(Session("TempDataset"), SchemaType.Source, "SqHeader")
        LocalTempHeaderDataTable = Session("TempDataset").Tables("SqHeader")

        Dim TempItemDataRow As DataRow = Session("TempDataset").Tables("SqHeader").NewRow()
        ...
        SqlAdapterHeader.Update(Session("TempDataset"), "SqHeader")
        ...
        Try
            ...
            If Not IsNothing(Session("TempDataTable")) Then
                If Session("TempDataTable").Rows.Count > 0 Then
                    ...
                    LocalTempDataTable = Session("TempDataTable")
                    For Each ro As DataRow In LocalTempDataTable.Rows
                        Dim InsertCommand As SqlCommand = New SqlCommand()
                        InsertCommand.Connection = Business_SQ.FocusDBConn
                        InsertCommand.Transaction = SqlTrans
                        InsertCommand.CommandText = " INSERT INTO SqLineItems" & _
                        "([SNo]" & _
                       ",[HeaderId]" & _
                       ",[ItemCode]" & _
                       ",[ItemDescription]" & _
        ...
        If Not IsNothing(Session("TempDataTable")) Then
            If Session("TempDataTable").Rows.Count > 0 Then
                ...
                For Each DataRowItem As DataRow In Session("TempDataTable").Rows
                    Dim DR As DataRow = Session("TempDataset").Tables("SqLineItems").Rows(i)
                ...
                Next
                For Each DataRowItem As DataRow In Session("TempDataTable").Rows
                    Dim TempDataRow As DataRow = Session("TempDataset").Tables("SqLineItems").NewRow()
                    TempDataRow = DataRowItem
                    Dim CB As SqlCommandBuilder = New SqlCommandBuilder(SqlAdapter)
                    SqlAdapter.Update(Session("TempDataset"), "SqLineItems")
                Next
                SqlAdapter.Dispose()
    ...
    Protected Function UpdateHeaderNew(ByVal InternalDocId As String) As Integer
        Try
            ...
            Dim UpdateCommand As SqlCommand = New SqlCommand()
            UpdateCommand.Connection = Business_SQ.FocusDBConn
            UpdateCommand.CommandText = " UPDATE  SqHeader  SET " & _
           "CardCode=@CardCode" & _
           ",CardName=@CardName" & _
           ",ContactPerson=@ContactPerson" & _
           ",CustRefNo=@CustRefNo" & _
           ",PostingDate=@PostingDate" & _
           ",ValidUntil=@ValidUntil" & _
          " ,DocDate=@DocDate" & _
           ",DocRate=@DocRate" & _
           ",DocCurrency=@DocCurrency" & _
           ",DocDiscount=@DocDiscount" & _
           ",AddressId=@AddressId" & _
            ",Address=@Address" & _
           ",U_Validity=@U_Validity" & _
           ",U_Delivery_Term=@U_Delivery_Term" & _
           ",U_Shipped_From=@U_Shipped_From" & _
           ",U_Shipped_To=@U_Shipped_To" & _
           ",U_Trcking_ID=@U_Trcking_ID" & _
           ",U_End_User=@U_End_User" & _
           ",U_Remarks=@U_Remarks" & _
           ",U_Delivery_Date=@U_Delivery_Date" & _
           ",U_Country_of_origin=@U_Country_of_origin" & _
           ",U_Revision_No=@U_Revision_No" & _
           ",U_HSCode=@U_HSCode" & _
           ",U_Min_Charges=@U_Min_Charges" & _
           ",U_Layout=@U_Layout" & _
           ",PaymentTerm=@PaymentTerm" & _
           ",DocTotal=@DocTotal" & _
           ",DocTotalFC=@DocTotalFC" & _
           ",DiscPrcnt=@DiscPrcnt" & _
           ",DiscSum=@DiscSum" & _
           ",DiscSumFc=@DiscSumFc" & _
           ",VatSum=@VatSum" & _
           ",VatSumFC=@VatSumFC" & _
           ",UpdatedOn=@UpdatedOn" & _
           ",CreatedBy=@CreatedBy" & _
           "  OUTPUT INSERTED.id  WHERE " & _
            " id=@Headerid"
