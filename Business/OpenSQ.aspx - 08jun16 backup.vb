...
Partial Class Business_OpenSQ
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If ListofDocs <> String.empty Then
            SqlDataSource1.SelectCommand = "SELECT CASE WHEN ISNULL(docnum,'')='' THEN id ELSE DocNum END AS  DocNum  , [DocStatus], [CardCode], [CardName], [ValidUntil], [DocDate] FROM [SqHeader] WHERE ([DocStatus] IS NULL) AND [id] IN (" + GetListofDocs() + ")  AND [CardName] LIKE @CardName"
        Else
            SqlDataSource1.SelectCommand = "SELECT CASE WHEN ISNULL(docnum,'')='' THEN id ELSE DocNum END AS  DocNum  , [DocStatus], [CardCode], [CardName], [ValidUntil], [DocDate] FROM [SqHeader] WHERE ([DocStatus] IS NULL) AND createdBy=@createdby  AND [CardName] LIKE @CardName"
